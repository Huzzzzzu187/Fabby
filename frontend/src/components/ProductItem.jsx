...
const ProductItem = ({ id, image, name, price }) => {
  ...
  const { currency, products, getImageUrl } = useContext(ShopContext)
  ...
  return (
    <Link className='group' to={`/product/${id}`}>
      <div className="relative aspect-square rounded-3xl overflow-hidden bg-slate-100 mb-4">
        <img className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" src={getImageUrl(image[0])} alt={name} />
