...
const PlaceOrder = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      for (const itemId in cartItems) {
        for (const variantKey in cartItems[itemId]) {
          if (cartItems[itemId][variantKey] > 0) {
            ...
            if (itemInfo) {
              const [size, subscription] = variantKey.split('_');
              let discountMultiplier = 1;

              if (subscription && subscription !== 'One-time') {
              ...
              itemInfo.price = itemInfo.price * discountMultiplier;
              itemInfo.size = size;
              itemInfo.subscription = subscription;
              itemInfo.quantity = cartItems[itemId][variantKey];
      ...
      if (hasSubscription && method === 'cod') {
        toast.error("Subscriptions cannot be paid via Cash on Delivery. Please select Stripe or Razorpay.");
        return;
      }
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
      {/* --------------------- LEFT SIDE ------------------- */}
        ...
        <div className='mt-12'>
          ...
          <div className='flex gap-3 flex-col lg:flex-row'>
            <div onClick={() => setMethod('stripe')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'stripe' ? 'bg-[var(--color-primary)]' : ''} `}></p>
              <img className='h-4 mx-4' src={assets.stripe_logo} alt="" />
            </div>

            <div onClick={() => setMethod('razorpay')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'razorpay' ? 'bg-[var(--color-primary)]' : ''} `}></p>
              <img className='h-4 mx-4' src={assets.razorpay_logo} alt="" />
            </div>

            <div onClick={() => setMethod('cod')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'cod' ? 'bg-[var(--color-primary)]' : ''} `}></p>
              <p className='text-gray-500 text-sm font-medium mx-4'>CASH ON DELIVERY</p>
            </div>
          </div>

          <div className='w-full text-end mt-8 '>
            <button type='submit' className='bg-[var(--color-primary)] hover:bg-[var(--color-primary)]/90 text-white rounded-xl font-bold px-16 py-3.5 text-sm cursor-pointer shadow-lg shadow-[var(--color-primary)]/20 transition-all'>PLACE ORDER</button>
